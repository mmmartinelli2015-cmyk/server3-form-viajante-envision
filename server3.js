...
function normalizeCustomFields(input) {
  ...
  if (Array.isArray(input)) {
    ...
    for (const item of input) {
      const key = safeString(item?.custom_field_api_identifier || item?.custom_field_id || item?.name || item?.key);
...
const COUNTRY_MAP = {
  ...
  'alemanha': 'DE', 'reino unido': 'GB', 'inglaterra': 'GB', 'escócia': 'GB',
  ...
  'singapura': 'SG', 'malásia': 'MY', 'filipinas': 'PH',
...
function countryNameToCode(value) {
  ...
  // Se já é código ISO (2 letras maiúsculas)
  if (/^[A-Z]{2,3}$/.test(value.trim())) return value.trim();
  return COUNTRY_MAP[v] || value.trim() || 'BR';
}
...
function buildDocuments(formData, fullName) {
  ...
  // RG — Type '0' conforme Envision (0=RG, 2=CPF, 1=Passaporte, 3=CNH, 5=RNE)
  if (safeString(formData.rg)) {
    docs.push(removeEmptyDeep({
      Type: '0',
      TypeName: 'RG',
      FullName: fullName,
      ...
      Country: 'BR',
      CountryName: 'Brasil',
      ExpireDate: buildExpireDate(formData.dataExpiracao) || {
  ...
  if (safeString(formData.cpf)) {
    docs.push({
      ...
      Country: 'BR',
      CountryName: 'Brasil',
      ExpireDate: { year: 2099, month: 1, day: 1, dayOfWeek: 'thursday', hour: 0, minutes: 0, seconds: 0, millisecond: 0 }
    });
  }

  // Passaporte brasileiro — Type '1'
  ...
  if (safeString(formData.numPassaporteEstrangeiro)) {
    ...
    docs.push(removeEmptyDeep({
      ...
      ExpireDate: buildExpireDate(formData.dataExpiracaoEstrangeiro)
    }));
  }
  ...
  return removeEmptyDeep(docs);
}

// ======================= ENDERECOS =======================
function buildAddresses(formData) {
  const hasAddress =
    safeString(formData.endereco) || safeString(formData.numero) ||
    safeString(formData.complemento) || safeString(formData.bairro) ||
    safeString(formData.cidade) || safeString(formData.estado) ||
    safeString(formData.cep) || safeString(formData.pais);
  if (!hasAddress) return [];
  return [removeEmptyDeep({
    Type: '0', TypeName: 'Residencial',
...
function buildEmergencyContacts(formData) {
  const contacts = [];
  const parsed = parseGridItems(pickFirst(formData.emergencyContacts, formData.EmergencyContacts, formData.gridEmergencyContacts));
...
function buildVaccines(formData) {
  const vaccines = [];
  const parsed = parseGridItems(pickFirst(formData.vaccines, formData.Vaccines, formData.vacinasGrid));
  for (const item of parsed) {
    const normalized = removeEmptyDeep({
...
function buildCustomFields(formData, firstName, middleName, lastName, genderName) {
  ...
  addAliases(cf, ['medicamentoContinuo', 'MedicamentoContinuo', 'medicamento_continuo'], boolToSimNao(boolToBoolean(formData.medicamentoContinuo)));
  addAliases(cf, ['medicamentoContinuoTexto', 'medicamento_continuo_detalhes', 'Person.Medicine', 'Medicine', 'Medicine.Swim', 'medicamento_detalhes', 'medicamento_dosagem'], safeString(formData.medicamentoContinuoTexto));
  ...
  addAliases(cf, ['alergiasTexto', 'alergias_detalhes'], safeString(formData.alergiasTexto));
  ...
  addAliases(cf, ['restricoesAlimentares', 'restricoes_alimentares', 'Person.Diet'],
    safeString(formData.restricoesAlimentares || formData.restricaoAlimentos || formData.alergiaAlimentos || formData.alimentosNaoCome));
  ...
  addAliases(cf, ['alergiaAlimentos', 'AlergiaAlimentos', 'alergia_alimentos'], safeString(formData.alergiaAlimentos));
  addAliases(cf, ['restricaoAlimentos', 'RestricaoAlimentos', 'restricao_alimentos'], safeString(formData.restricaoAlimentos));
  addAliases(cf, ['alimentosNaoCome', 'AlimentosNaoCome', 'alimentos_nao_come',
    'alimentos_que_nao_come', 'descreva_alimentos_que_voce_nao_come_por_gosto_dieta_ou_restricao',
    'Person.DislikedFood'], safeString(formData.alimentosNaoCome));
  ...
  addAliases(cf, ['receberCatalogos', 'receber_catalogos',
    'gostaria_de_receber_os_nossos_catalogos_fisicos', 'Person.Catalog'],
    safeString(formData.receberCatalogos));
  ...
  addAliases(cf, ['comentarios', 'Comentarios', 'comentarios_extras',
    'comentarios_ou_informacoes_extras_que_considere_necessarios',
    'observacoes', 'informacoes_adicionais', 'Person.Comments'],
...
function normalizeWebhookPayload(body) {
  ...
  return {
    ...
    medicamentoContinuo:      safeString(pickFirst(cf.medicamento_continuo)).toLowerCase() === 'sim',
    medicamentoContinuoTexto: pickFirst(cf.medicamento_continuo_detalhes, cf.medicamento_continuo_texto, cf['medicamento_continuo_texto'], cf['Person.Medicine'], cf['Medicine.Swim']),
    ...
    alergias:                 safeString(pickFirst(cf.alergias)).toLowerCase() === 'sim',
    alergiasTexto:            pickFirst(cf.alergias_detalhes),
    ...
    restricoesAlimentares: pickFirst(cf.restricoes_alimentares, cf['Person.Diet']),
    dieta:                 safeString(pickFirst(cf.dieta)).toLowerCase() === 'sim',
    dietaTexto:            pickFirst(cf.dieta_detalhes, cf['Person.DietQuestion']),
    alergiaAlimentos:      pickFirst(cf.alergia_alimentos),
    alimentosNaoCome:      pickFirst(cf.alimentos_nao_come, cf.alimentos_que_nao_come,
    ...
    comentarios:             pickFirst(cf.comentarios, cf.comentarios_extras, cf.comentarios_ou_informacoes_extras_que_considere_necessarios, cf.observacoes, cf.informacoes_adicionais, cf['Person.Comments']),
...
async function getEnvisionToken() {
  if (!ENVISION_USERNAME || !ENVISION_PASSWORD) throw new Error('Credenciais nao configuradas');
...
function buildPostPayload(formData, existingId = null) {
  ...
  return removeEmptyDeep({
    companyContext: {
      consolidator: { id: ENVISION_CONSOLIDATOR_ID, systemAccountId: ENVISION_CONSOLIDATOR_SYSTEM_ACCOUNT_ID },
      travelAgency: { id: ENVISION_TRAVEL_AGENCY_ID, systemAccountId: ENVISION_TRAVEL_AGENCY_SYSTEM_ACCOUNT_ID }
    },
    record: {
      ...(existingId ? { id: existingId } : {}),
      type: ENVISION_RECORD_TYPE, systemAccountId: ENVISION_SYSTEM_ACCOUNT_ID, travelAgencyId: ENVISION_TRAVEL_AGENCY_ID, active: true,
      identification, summary: `${nomeCompleto || 'Lead sem nome'} - Person Record`, externalId: cpf.replace(/\D/g, '') || email,
      firstName: safeString(formData.chamado) || firstName, middleName, lastName, email, phone: telefone, gender, genderName,
      nationality: safeString(formData.nacionalidade) || 'Brasil',
      city: safeString(formData.cidade), adress: safeString(formData.endereco),
      adressNumber: safeString(formData.numero), zipCode: safeString(formData.cep), uF: safeString(formData.estado),
      birthDate: parseDateParts(formData.nascimento),
      documents: buildDocuments(formData, nomeCompleto),
      addresses: buildAddresses(formData),
      EmergencyContacts: emergencyContacts, Vaccines: vaccines,
      Height: safeString(formData.altura), Weight: safeString(formData.peso),
      ShoeNumber: safeString(formData.numeroCalcado), ClothingNumber: safeString(formData.tamanhoRoupa),
      Languages: safeString(formData.idiomas), Country: safeString(formData.pais) || 'Brasil',
      District: safeString(formData.bairro), PreferredName: safeString(formData.chamado),
      Press: safeString(formData.midia), Agency: safeString(formData.agenciaViagens), SeatPreference: safeString(formData.preferenciaAssento), BloodType: safeString(formData.tipoSanguineo),
      LastExamDate: parseDateParts(formData.dataCheckup), LastCheckUp: parseDateParts(formData.dataCheckup),
      Healthy: safeString(formData.saudavel), PhysicalActivity: safeString(formData.atividadeFisica),
      Swim: safeString(formData.sabeNadar), Catalog: safeString(formData.receberCatalogos),
      CatalogQuestion: safeString(formData.enderecoPostalIgual), CatalogQuestionAddress: safeString(formData.enderecoPostalDiferente),
      Condition: safeString(formData.restricaoFisicaTexto || boolToSimNao(boolToBoolean(formData.restricaoFisica))),
      Illness: safeString(formData.doencaCronicaTexto || boolToSimNao(boolToBoolean(formData.doencaCronica))),
      Medicine: safeString(formData.medicamentoContinuoTexto) || boolToSimNao(boolToBoolean(formData.medicamentoContinuo)),
      Surgery: safeString(formData.motivoCirurgia || boolToSimNao(boolToBoolean(formData.cirurgia))),
      MedicalQuestion: safeString(formData.questaoMedicaTexto || boolToSimNao(boolToBoolean(formData.questaoMedica))),
      ...
      Diet: safeString(formData.restricoesAlimentares || formData.restricaoAlimentos || formData.alergiaAlimentos || formData.alimentosNaoCome),
      DietQuestion: safeString(formData.dietaTexto || boolToSimNao(boolToBoolean(formData.dieta))),
      DislikedFood: safeString(formData.alimentosNaoCome), Comments: safeString(formData.comentarios), Other: safeString(formData.outros),
      permissions: { edit: true, onlineRetrieve: true }, customFields
    }
  });
}

// ======================= BUILD PAYLOAD PATCH =======================
// Envia apenas campos novos que o server4 nao enviou
function buildPatchPayload(formData, existingId) {
  const cpf   = safeString(formData.cpf);
  const email = safeString(formData.email);

  // Apenas campos complementares — nao repete o que server4 ja enviou
  const customFields = removeEmptyDeep({
    ...
    // Alimentacao
    restricoesAlimentares:       safeString(formData.restricoesAlimentares),
    restricoes_alimentares:      safeString(formData.restricoesAlimentares),
    'Person.Diet':               safeString(formData.restricoesAlimentares),
    ...
    alergiaAlimentos:            safeString(formData.alergiaAlimentos),
    alergia_alimentos:           safeString(formData.alergiaAlimentos),
    alimentosNaoCome:            safeString(formData.alimentosNaoCome),
    alimentos_nao_come:          safeString(formData.alimentosNaoCome),
    'Person.DislikedFood':       safeString(formData.alimentosNaoCome),
...
async function sendFormToEnvision(formData) {
  ...
  // 1. Verifica o records-map salvo pelo server4
  let existingId = findRecordIdInMap(cpf, email);
  if (existingId) {
    console.log(`[RecordsMap] ID encontrado no mapa local: ${existingId}`);
  }

  // 2. Se nao encontrou no mapa, tenta Records/Query como fallback
  ...
  for (const candidate of candidates) {
    try {
      ...
      const body = {
        ...
        sortFields: [], pagingPivotId: 0, pagingPivotValues: [],
        additionalInfo: { travelAgencyId: ENVISION_TRAVEL_AGENCY_ID, systemAccountId: ENVISION_SYSTEM_ACCOUNT_ID }
      };
      const resp = await fetch(url, {
        method: 'POST',
      ...
      if (resp.ok && data?.successful !== false) {
        ...
        if (records[0]?.id) { existingId = records[0].id; break; }
      }
    } catch (err) {
      console.warn(`[Envision] Erro Records/Query (${candidate}):`, err.message);
    }
  }
  } // fim fallback Records/Query
  ...
  // Nenhum registro encontrado — cria novo via POST
...
// ======================= ROTAS =======================
app.get('/', (req, res) => res.send('Servidor RD Webhook + Envision /Records (Person) online.'));

app.get('/health', async (req, res) => {
  try {
    if (!RD_CRM_API_TOKEN) return res.status(400).json({ success: false, error: 'RD_CRM_API_TOKEN nao configurado' });
    const response = await fetch('https://crm.rdstation.com/api/v1/token/check', {
      method: 'GET',
      headers: { Authorization: `Token token=${RD_CRM_API_TOKEN}`, 'Content-Type': 'application/json' }
    });
...
app.post('/rdstation-webhook', async (req, res) => {
  try {
    ...
    if (!formData.nomeCompleto || !formData.email) {
      return res.status(200).json({ success: false, error: 'Campos obrigatorios ausentes.', normalized: formData });
    }
    try {
      const envisionResult = await sendFormToEnvision(formData);
      return res.status(200).json({ success: true, message: 'Dados enviados ao Envision com sucesso.', normalized: formData, envision: envisionResult });
    } catch (err) {
      console.error('[Webhook] Erro ao enviar ao Envision:', err.message);
      return res.status(200).json({ success: false, error: 'Erro ao integrar com Envision', detail: err.message, normalized: formData });
    }
  } catch (err) {
    console.error('[Webhook] Erro inesperado:', err.message);
    return res.status(200).json({ success: false, error: 'Erro interno', detail: err.message });
  }
});
...
app.post('/envision/form', async (req, res) => {
  try {
    ...
    const envisionResult = await sendFormToEnvision(formData);
    return res.status(200).json({ success: true, normalized: formData, envision: envisionResult });
  } catch (err) {
    return res.status(500).json({ success: false, error: err.message });
  }
});
